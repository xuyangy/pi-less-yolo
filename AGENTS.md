...
| `omp-lsp.yml` | LSP server definitions omp has no defaults for, baked in at `/home/ompuser/.lsp.yml`. That is the LOWEST-priority config source (project `lsp.*` > `.omp|.pi|.claude/lsp.*` > `~/.omp/agent/lsp.*` > `~/lsp.*`), so a project's own config still wins. Adds `ansiblels` (the vscode-ansible server) pointed at `/opt/omp-python`, and disables omp's `pyright` entry — basedpyright's package also installs `pyright-langserver`, which would otherwise start a second server and double every Python diagnostic. Shipping any config file switches `loadConfig` from its auto-detect branch to merge-and-filter; the two filter identically, the latter also honours `disabled`. |
...
| Language servers (npm) and `ruff` / `ansible-lint` (PyPI) | pins carrying a `# renovate: datasource=… depName=…` marker in `Dockerfile.omp` | npm, PyPI |

The language-server pins share **one** marker-driven custom manager rather than an entry each. The marker is what keeps it off the `@oh-my-pi/*` lines above (they carry none), so the two managers cannot both claim the same pin. Two deliberate exceptions: `typescript` is capped at `<6` by a `packageRules` entry, because it exists only to give `typescript-language-server` a `tsserver` to drive and the 7.x package (the native port) no longer ships one; and `lua-language-server` has no marker at all, since its version and its two per-arch SHA-256 digests must move together and a version-only PR would fail the checksum — bump it by hand.
