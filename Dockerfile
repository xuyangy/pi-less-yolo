...
&& MISE_VERSION=2026.8.10 MISE_INSTALL_PATH=/usr/local/bin/mise sh /tmp/mise-install.sh \
...
RUN set -e \
&& mise install uv@0.12.5 \
&& ln -s "$(mise exec uv@0.12.5 -- which uv)" /usr/local/bin/uv \
&& ln -s "$(mise exec uv@0.12.5 -- which uvx)" /usr/local/bin/uvx
